      ...
      <div className="flex flex-wrap justify-center my-2">
        {CategoryInformation.map((info) => (
          <div className="relative mr-3 mt-2" key={info.id}>
            <img
              className="sm:max-w-sm max-w-xs object-contain"
            ...
            <div className="flex justify-center tracking-widest">
              <Link
                to={info.redirect}
                style={{ color: "#000" }}
                className="absolute bottom-0 my-5 text-black border py-2 px-8 sm:text-sm text-xs bg-white font-bold hover:opacity-75 uppercase"
              >
                {info.buttonName}
              </Link>
            </div>
