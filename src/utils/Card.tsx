...
interface PropsType {
    ...
    onClick? : (event : React.MouseEvent<HTMLLIElement, MouseEvent>, id:string) => void
}

const Card :React.SFC<PropsType> = ({filteredItems,onClick}) => {

    return(
        <Flipmove typeName="ul" className="grid grid-rows md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
             {filteredItems.map((item : any) => (
                item.quantity > 0 && (
                    <li className="border mt-5 mr-2 cursor-pointer" key={item.id} 
                        ...
                        <div className="px-4 py-2 font-segoe-UI">
                            <div className="flex items-center justify-between">
                                ...
                                {item.status !== undefined ? (
                                    <span className="rounded-full bg-gray-500 h-2 w-2 mb-3" style={{backgroundColor: item.status['color']}}></span>
                                ) : (
